...
#define hfov (0.73f * WINDOW_HEIGHT)  // Affects the horizontal field of vision
#define vfov (.2f * WINDOW_HEIGHT)    // Affects the vertical field of vision
...
static struct sector {
	...
	signed char  *neighbors;
	              //Each edge may have a corresponding neighboring sector
	unsigned npoints;
...
/*
 * MovePlayer(dx,dy): Moves the player by (dx,dy) in the map, and also
 * updates their anglesin/anglecos/sector properties properly.
 */
static void MovePlayer(float dx, float dy) {
	float px = player.where.x, py = player.where.y;
	/*
	 * Check if this movement crosses one of this sector's edges that
	 * have a neighboring sector on the other side. Because the edge
	 * vertices of each sector are defined in clockwise order, PointSide
	 * will always return -1 for a point that is outside the sector and 0
	 * or 1 for a point that is inside.
	 */
...
static void DrawScreen() {
	...
	int	ytop[WINDOW_WIDTH] = {0}, ybottom[WINDOW_WIDTH], renderedsectors[NumSectors];
	for (unsigned x = 0; x < WINDOW_WIDTH; ++x)
		ybottom[x] = WINDOW_HEIGHT - 1;
	for (unsigned n = 0; n < NumSectors; ++n)
		renderedsectors[n] = 0;

	/* Begin whole-screen rendering from where the player is. */
	*head = (struct item){
		player.sector, 0, WINDOW_WIDTH - 1
	};
	...
	do {
		...
		if (renderedsectors[now.sectorno] & 0x21)
			continue;
		//Odd = still rendering, 0x20 = give up
			++ renderedsectors[now.sectorno];
		const struct sector *const sect = &sectors[now.sectorno];
		/*
		 * Render each wall of this sector that is facing towards
		 * player.
		 */
		for (unsigned s = 0; s < sect->npoints; ++s) {
			/*
			...
			/*
			 * Is the wall at least partially in front of the
			 * player?
			 */
			if (tz1 <= 0 && tz2 <= 0)
				continue;
			/*
			 * If it's partially behind the player, clip it
			 * against player's view frustrum
			 */
			if (tz1 <= 0 || tz2 <= 0) {
			...
			int	x1 = WINDOW_WIDTH / 2 - (int)(tx1 * xscale1);
			float xscale2 = hfov / tz2, yscale2 = vfov / tz2;
			int	x2 = WINDOW_WIDTH / 2 - (int)(tx2 * xscale2);
			...
			/*
			 * Project our ceiling & floor heights into screen
			 * coordinates (Y coordinate)
			 */
			
			int	y1a = WINDOW_HEIGHT / 2 - (int)(Yaw(yceil, tz1) * yscale1),
			y1b = WINDOW_HEIGHT / 2 - (int)(Yaw(yfloor, tz1) * yscale1);
			int	y2a = WINDOW_HEIGHT / 2 - (int)(Yaw(yceil, tz2) * yscale2),
			y2b = WINDOW_HEIGHT / 2 - (int)(Yaw(yfloor, tz2) * yscale2);
			/* The same for the neighboring sector */
			int	ny1a = WINDOW_HEIGHT / 2 - (int)(Yaw(nyceil, tz1) * yscale1),
			ny1b = WINDOW_HEIGHT / 2 - (int)(Yaw(nyfloor, tz1) * yscale1);
			int	ny2a = WINDOW_HEIGHT / 2 - (int)(Yaw(nyceil, tz2) * yscale2),
			ny2b = WINDOW_HEIGHT / 2 - (int)(Yaw(nyfloor, tz2) * yscale2);

			/* Render the wall. */
			int	beginx = max(x1, now.sx1), endx = min(x2, now.sx2);
			for (int x = beginx; x <= endx; ++x) {
				/*
				 * Calculate the Z coordinate for this point.
				 ...
				 * sector's ceiling height.
				 */
				vline(x, ytop[x], cya - 1, 255, 0, 0);
				/*
				 * Render floor: everything below this
				 * sector's floor height.
				 */
				vline(x, cyb + 1, ybottom[x], 0, 0, 255);
				...
				if (neighbor >= 0) {
					...
					/*
					 * If our ceiling is higher than
					 * their ceiling, render upper wall
					 */
					vline(x, cya, cnya - 1, 0, 255, 0);
					//Between our and their ceiling
					ytop[x] = clamp(max(cya, cnya), ytop[x], WINDOW_HEIGHT - 1);
					//Shrink the remaining window below these ceilings
					/*
					 * If our floor is lower than their
					 * floor, render bottom wall
					 */
					vline(x, cnyb + 1, cyb, 0, 255, 0);
					//Between their and our floor
					ybottom[x] = clamp(min(cyb, cnyb), 0, ybottom[x]);
					//Shrink the remaining window above these floors
				} 
				else {
					vline(x, cya, cyb, 0, 255, 0);
				}
			}
			/*
...
int main() {
	...
	window = SDL_CreateWindow("styx engine", 0, 0, WINDOW_WIDTH, WINDOW_HEIGHT, 0);
	...
	for (;;) {
		...
		if (moving) {
			...
			const struct xy *const vert = sect->vertex;
			/*
			 * Check if the player is about to cross one of the
			 * sector's edges
			 */
			for (unsigned s = 0; s < sect->npoints; ++s)
